...
public class ExceptionHandling {
	...
	public static void main(String[] args) {
		 ...
		 *  -논리적 에러 : 실행은 되지만, 의도와 다른게 동작하는것 (버그)
		 *  -런타입 에러 : 실행 시에 발생되는 에러(우리가 처리할 에러)
		 *  
		 *  << 런타임 에러 >>
		 *  -런타임 에러 발생시 발생한 위치에서 프로그램이 비정상적으로 종료된다
		 *  -에러 : 프로그램 코드에 의해서 수습될 수 없는 심각한 우류(처리불가)
		 ...
		 *  -[RuntimeException 클래스와 그 자식들을 제외한] Exception 클래스 자식들은 예외처리가 강제된다 (checked예외)
		 *  
		 *  << 예외처리(try-catch) >>
		 ...
		 */
		try{
		int result = 10 / 0; //예외가 발생한 위치에서 프로그램이 비정상적으로 종료됨
		System.out.println(result); //예외 발생 시 실행되지 않는다
		} catch(ArithmeticException | IndexOutOfBoundsException e){
			//예외 발생 시 발생한 예외와 일치하는 catch블럭이 수행되고
			//일치하는 catch가 없는 경우 예외는 처리되지 않는다
			e.printStackTrace();  //<-에러메세지 그대로 출력하기 위해

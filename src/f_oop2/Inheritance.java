...
public class Inheritance {
	...
	public static void main(String[] args) {
		 ...
		 *  -자식 클래스 생성자의 척줄에 super()가 있어야한다
		 *  -없으면 컴파일러가 자동으로 추가한다
		 *  
		 *  << 오버라이딩 >>
		 *  -상속받은 메서드의 내용을 재정의하는 것이다
		 *  -상속받은 메서드와 리턴타입, 메서드명, 파라미터가 같아야 한다
		 *  -상속받은 메서드 보다 좁은 범위의 접근제어자로 변경할 수 없다
		...
		System.out.println(am.publicVar);  //유효성 체크 잘못된 값을 막기 위해서
		am.publicMethod();
...
//		t.setSecond(39);
		...
		System.out.println(Singleton.getInstance());   //싱글톤은 다 겟인스턴스를 가지고있슴
		System.out.println(Singleton.getInstance());
		System.out.println(Singleton.getInstance());  //계속 같은 주소
		
		System.out.println(new Inheritance());
		System.out.println(new Inheritance());
		System.out.println(new Inheritance());  //다른주소 반환

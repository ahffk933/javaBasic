...
/*		[7-3] 오버라이딩의 정의와 필요성에 대해서 설명하시오.
		상속받은 메서드 내용을 수정할 수 있음/ 상속받은 메서드와 리턴타입, 메서드명, 파라미터는 같아야함
		...
		Tv() {}   //파라미터가 있는 프로덕트밖에 없어서 애를 불러오려면 파라미터값이 없는 프로덕츠 생성자를 만들어줘야함
		...
		[7-7] 다음 코드의 실행했을 때 호출되는 생성자의 순서와 실행결과를 적으시오.  200
		...
		class Child extends Parent {
			int x = 3000;
			Child() {
				this(1000);  //1   //this가 있는 곳에 super올수 x
			}
			Child(int x) {       //생략된 super
				this.x = x;  //2
		}
		}
		class Exercise7_7 {
		...
		옳지 않은 것은? (모두 고르시오) c -> override할수 있슴
		a. 지역변수 - 값을 변경할 수 없다.
	...
	public void gotoPrevChannel(){
		...
		[7-13] Math클래스의 생성자는 접근 제어자가 private이다. 그 이유는 무엇인가?
			클래스 내에 인스턴스 변수가 하나도 없어서 인스턴스를 생성할 필요가 없기 때문, 
			java.lang에 포함되어 있어서 import없이 쓸 수 있슴
		...
		[7-15] 클래스가 다음과 같이 정의되어 있을 때, 형변환을 올바르게 하지 않은 것은?
			...
			d. AirUnit au = ac;
			e. t = (Tank)u;  //조상타입 인스턴스를 자손타입으로 형변환 x
			f. GroundUnit gu = t;
		...
		[7-16] 다음 중 연산결과가 true가 아닌 것은? (모두 고르시오) e
			...
			d. fe instanceof Car
			e. fe instanceof Ambulance  //얘빼고 참조변수 형변환 가능~
		...
		[7-20] 다음의 코드를 실행한 결과를 적으시오.  p.x=100 p.c =200
	...
	class Child extends Parent {
	int x = 200;
	void method() {    //오버라이딩 된거 먼저 실행 됨
	System.out.println("Child Method");
	}
	}
	[7-24] 다음 중 인터페이스의 장점이 아닌 것은? d,e
	a. 표준화를 가능하게 해준다.
	b. 서로 관계없는 클래스들에게 관계를 맺어 줄 수 있다.
	c. 독립적인 프로그래밍이 가능하다.
	d. 다중상속을 가능하게 해준다.  //단점이 더 많다고 팡단하여 허용 x
	e. 패키지간의 연결을 도와준다.
